...
export const EMPTY_STATE = { paused: [], lastRunAt: {} };
...
export function loadState(projectDir) {
    ...
    try {
        ...
        if (parsed.lastRunAt !== null && typeof parsed.lastRunAt === 'object') {
            for (const [name, value] of Object.entries(parsed.lastRunAt)) {
                if (typeof value === 'number' && Number.isFinite(value))
                    lastRunAt[name] = value;
            }
        }
        return {
            paused: Array.isArray(parsed.paused) ? parsed.paused.filter((p) => typeof p === 'string') : [],
            lastRunAt,
        };
    }
    catch {
