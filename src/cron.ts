...
import { isValidTimeZone, wallParts, wallToInstant } from './time.ts'
...
export function parseSchedule(rawSpec: string): RoutineSchedule {
  const spec = rawSpec.trim().toLowerCase()
  if (spec === '') throw new Error('schedule is empty')
  ...
  if (every !== null) {
    ...
    if (!Number.isSafeInteger(n) || n <= 0) throw new Error(`invalid "every" schedule ${JSON.stringify(rawSpec)}`)
    const multiplier = unit.startsWith('s') ? 1_000 : unit.startsWith('m') ? 60_000 : 3_600_000
    const intervalMs = n * multiplier
    if (intervalMs < 60_000) throw new Error(`"every" interval must be at least 1 minute (got ${JSON.stringify(rawSpec)})`)
...
function parseField(field: string, min: number, max: number, allowQuestion: boolean, rawSpec: string): Set<number> | null {
  if (field === '*') return null
  if (allowQuestion && field === '?') return null
  const values = new Set<number>()
  for (const piece of field.split(',')) {
    if (piece === '') throw new Error(`schedule ${JSON.stringify(rawSpec)} has an empty field element`)
    let step = 1
    let range = piece
    ...
    if (stepMatch !== null) {
      ...
      if (!Number.isSafeInteger(step) || step <= 0) throw new Error(`schedule ${JSON.stringify(rawSpec)} has an invalid step ${JSON.stringify(piece)}`)
    }
    let lo: number
    let hi: number
    if (range === '*') {
      ...
      if (rangeMatch !== null) {
        ...
        if (lo > hi) throw new Error(`schedule ${JSON.stringify(rawSpec)} has a reversed range ${JSON.stringify(piece)}`)
      } else {
  ...
  for (const v of values) normalized.add(isDow && v === 7 ? 0 : v)
  if (normalized.size === 0) throw new Error(`schedule ${JSON.stringify(rawSpec)} matches nothing`)
  return normalized
}

/** Resolve one atom (number or name) within a field's range. */
function parseAtom(atom: string, min: number, max: number, rawSpec: string): number {
  let value: number
  if (/^\d+$/.test(atom)) {
    value = Number(atom)
  } else {
    value = (max === 12 ? MONTH_NAMES : max === 7 ? DOW_NAMES : {})[atom] ?? Number.NaN
    if (Number.isNaN(value)) throw new Error(`schedule ${JSON.stringify(rawSpec)} has an unknown field value ${JSON.stringify(atom)}`)
  }
 ...
 * horizon.
 */
export function nextAfter(schedule: RoutineSchedule, after: number, timeZone: string): number | null {
  if (!isValidTimeZone(timeZone)) throw new Error(`invalid timezone ${JSON.stringify(timeZone)}`)
  if (schedule.kind === 'every') return after + schedule.intervalMs
  return nextCronAfter(schedule, after, timeZone)
}

/** Binary-search the first minute-of-day (h*60+m) strictly greater than `target`. */
function firstCandidateAfter(candidates: readonly number[], target: number): number | undefined {
  let lo = 0
  let hi = candidates.length
  while (lo < hi) {
    const mid = (lo + hi) >> 1
    if ((candidates[mid] ?? 0) <= target) lo = mid + 1
    else hi = mid
  }
...
function nextCronAfter(schedule: Extract<RoutineSchedule, { kind: 'cron' }>, after: number, timeZone: string): number | null {
  const { minute, hour, dom, month, dow } = schedule
  // All valid (h*60+m) minute-of-day slots, sorted.
  const candidates: number[] = []
  for (const h of hour) for (const m of minute) candidates.push(h * 60 + m)
...
function nextDay(cursor: number, day: { year: number; month: number; day: number }, timeZone: string): number {
